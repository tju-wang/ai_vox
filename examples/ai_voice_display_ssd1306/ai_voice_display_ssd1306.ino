...
std::unique_ptr<Ssd1306Display> g_display;
...
void InitDisplay() {
  ...
  g_display = std::make_unique<Ssd1306Display>(panel_io, panel, kDisplayWidth, kDisplayHeight, kDisplayMirrorX, kDisplayMirrorY);
  g_display->Start();
}
  ...
  for (auto& event : events) {
    if (auto activation_event = std::get_if<ai_vox::Observer::ActivationEvent>(&event)) {
      ...
      switch (state_changed_event->new_state) {
        case ai_vox::Engine::State::kMqttConnecting: {
          g_display->ShowStatus("Mqtt connecting");
          break;
        }
        case ai_vox::Engine::State::kMqttConnected: {
